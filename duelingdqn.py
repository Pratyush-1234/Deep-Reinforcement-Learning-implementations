...
Original file is located at
    https://colab.research.google.com/drive/1Oxq7EIoNkmYe2jgqe-URro1vdOFhxMh2
"""
...
import numpy as np
os.makedirs("/tmp/dqn", exist_ok=True)
class ReplayBuffer(object):
  def __init__(self,max_size,input_shape,n_actions):
    ...
    self.terminal_memory=np.zeros(self.mem_size,dtype=np.uin)# unsigned integer 0-255 inclusive
...
class Agent(object):
  def __init__(self,gamma,epsilon,alpha,n_actions,input_dims,mem_size,batch_size,eps_min=0.01,eps_dec=5e-7,replace=1000, chkpt_dir='/tmp/dqn'):
    self.gamma = gamma
  ...
  def choose_action(self,observation):
    if np.random.random()>self.epsilon:
      observation=observation[np.newaxis,:]#column vector to row vector
